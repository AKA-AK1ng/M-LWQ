...
poly poly_add(const poly& a, const poly& b) {
    ...
    for (size_t i = 0; i < params::N; ++i) 
        res[i] = positive_mod((int32_t)a[i] + b[i], params::Q);
    return res;
}
...
poly poly_message_decode(const poly& val) {
    ...
    for (size_t i = 0; i < params::N; ++i) {
        res[i] = (val[i] > limit_low && val[i] < limit_high) ? 1 : 0;
    }
    return res;
}
...
poly_vec poly_matrix_vec_mul(const poly_matrix& A, const poly_vec& s) {
    ...
    for(int i=0; i<params::K; ++i) {
        ...
        for(int j=0; j<params::K; ++j) {
            poly tmp = poly_mul_mod(A[i][j], s[j]);
            acc = poly_add(acc, tmp);
        }
        res[i] = acc;
    }
    return res;
...
poly poly_vec_transpose_mul(const poly_vec& a_t, const poly_vec& b) {
    poly res(params::N, 0);
    for(int i=0; i<params::K; ++i) {
        res = poly_add(res, poly_mul_mod(a_t[i], b[i]));
    }
    return res;
}
