...
void poly_getnoise_eta1(poly *r, const uint8_t *seed, uint8_t nonce) {
    uint8_t buf[MLWQ_ETA1*MLWQ_N/4];
    uint8_t extseed[33];
...
// NTT adapter layer
// -------------------------------------------------------------------------
static void poly_ntt_internal(poly *r) { ntt(r->coeffs); }
static void poly_invntt_internal(poly *r) { invntt(r->coeffs); }

static void poly_basemul_internal(poly *r, const poly *a, const poly *b) {
...
// -------------------------------------------------------------------------
// Ciphertext U compression (10-bit)
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Ciphertext V compression (5-bit)
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Fast division constant (for Q=3329)
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Arithmetic & matrix multiplication (NTT-based)
// -------------------------------------------------------------------------
...
void poly_mul_ntt(poly *res, const poly *a, const poly *b) {
    poly ta = *a;
    poly tb = *b;
    poly_ntt_internal(&ta);
    poly_ntt_internal(&tb);
    poly_basemul_internal(res, &ta, &tb);
    poly_invntt_internal(res);
...
void poly_matrix_vec_mul(poly_vec *res, const poly_matrix *A, const poly_vec *s) {
    poly_vec s_ntt = *s;
    for(int i=0; i<MLWQ_K; i++) poly_ntt_internal(&s_ntt.vec[i]);
    ...
    for(int i = 0; i < MLWQ_K; ++i) {
        ...
        for(int j = 0; j < MLWQ_K; ++j) {
            ...
            poly t_prod;
            poly_ntt_internal(&t_a);
            poly_basemul_internal(&t_prod, &t_a, &s_ntt.vec[j]);
            for(int k=0; k<MLWQ_N; k++) acc.coeffs[k] += t_prod.coeffs[k];
        }
        poly_invntt_internal(&acc);
...
void poly_vec_transpose_mul(poly *res, const poly_vec *a_t, const poly_vec *b) {
    ...
    poly_vec b_ntt = *b;
    for(int i=0; i<MLWQ_K; i++) {
        poly_ntt_internal(&a_ntt.vec[i]);
        poly_ntt_internal(&b_ntt.vec[i]);
    }
    ...
    for(int i = 0; i < MLWQ_K; ++i) {
        poly t_prod;
        poly_basemul_internal(&t_prod, &a_ntt.vec[i], &b_ntt.vec[i]);
        for(int k=0; k<MLWQ_N; k++) acc.coeffs[k] += t_prod.coeffs[k];
    }
    poly_invntt_internal(&acc);
